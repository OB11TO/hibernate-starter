...
class UserDaoTest {
    ...
    void findAveragePaymentAmountByFirstAndLastName() {
        ...
        PaymentFilter filter = PaymentFilter.builder()
                ...
                .lastName("Gates")
                .build();
        Double averagePaymentAmount = userDao.findAveragePaymentAmountByFirstAndLastName(session, filter);
    ...
    void isItPossible() {
        ...
        List<String> names = results.stream().map(r -> Objects.requireNonNull(r.get(0, User.class)).fullName()).collect(toList());
